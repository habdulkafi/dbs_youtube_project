...
for ch in c_id[5:]:
	...
	if "likes" in playlists:
		playlistId = results["items"][0]["contentDetails"]["relatedPlaylists"]["likes"]
		...
		for vid in all_res:
			...
			try:
				...
				cur.execute(q)
				q = "INSERT INTO likes_2 (video_id, c_id) SELECT '{0}','{1}' WHERE NOT EXISTS (SELECT 1 FROM likes_2 WHERE video_id = '{0}' AND c_id = '{1}')".format(video_id,ch)
				cur.execute(q)
